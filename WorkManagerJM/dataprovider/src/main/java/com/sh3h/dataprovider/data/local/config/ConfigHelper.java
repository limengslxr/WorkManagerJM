...
public class ConfigHelper {
     ...
     * 根目录
     */
    public static final String FOLDER_ROOT = "sh3h/workmanagerjm";
    /**
     * 配置文件目录
     */
...
//            }
    ...
    public String getGrabUri() {
        return getSystemConfig().getString(SystemConfig.PARAM_SERVER_GRAB_URI);
    }
    ...
    public Observable<Boolean> savePicQuality(String str) {
        ...
        try {
            ...
            if (UserConfig.QUALITY_MIDDLE != integer && UserConfig.QUALITY_HIGH != integer) {
    ...
    public Observable<Boolean> isPhotoQualityNormal() {
        return Observable.create(subscriber -> {
            UserConfig userConfig = getUserConfig();
            int quality = userConfig.getInteger(UserConfig.PARAM_SYS_QUALITY_PHOTO, UserConfig.QUALITY_MIDDLE);
            subscriber.onNext(quality == UserConfig.QUALITY_MIDDLE);
            subscriber.onCompleted();
        });
    }
