 ...
 * 热线工单
 */
public class HotListAdapter extends AbstractAdapter {

    @Override
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_LOADING) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_loading, parent, false);
            return new HotListAdapter.LoadingHolder(view);
        }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
        if (viewHolder instanceof HotHolder) {
            HotHolder holder = (HotHolder) viewHolder;
            DUData duData = list.get(position);
            DUTask duTask = duData.getDuTask();
            holder.tvTaskId.setText(duTask.getHotTaskId() + File.separator + duTask.getCardId());
            holder.tvCardName.setText(duTask.getCardName());
            holder.tvAddress.setText(duTask.getAddress());
            holder.tvResponseType.setText(duTask.getResponseType());
            ...
            holder.tvMark.setVisibility(!TextUtil.isNullOrEmpty(original) && original.contains("12345")
            ...
            holder.tvTaskId.setTextColor(duTask.getEndDate() > System.currentTimeMillis()
    ...
    private class HotHolder extends RecyclerView.ViewHolder{
        private TextView tvTaskId, tvMark, tvCardName, tvAddress, tvResponseType, tvResolvePerson,
        ...
        HotHolder(View view) {
            ...
            tvResponseType = (TextView) view.findViewById(R.id.tv_item_response_type);
