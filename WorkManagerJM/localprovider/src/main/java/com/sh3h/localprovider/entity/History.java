...
public class History {
    ...
    @ToMany(joinProperties = {@JoinProperty(name = "taskId", referencedName = "id")})
    ...
    @Generated(hash = 331475967)
    public History(Long id, String account, long taskId, int type, int subType, int state,
            int hangUpState, int delayState, int reportType, int assist, String content,
            String reply, long replyTime, int uploadFlag, String extend) {
        this.id = id;
    ...
    public List<MultiMedia> getMultiMedias() {
        if (multiMedias == null) {
            ...
            List<MultiMedia> multiMediasNew = targetDao._queryHistory_MultiMedias(taskId,
                    state, replyTime);
            synchronized (this) {
                if (multiMedias == null) {
    ...
    public List<History> getHistories() {
        if (histories == null) {
            ...
            List<History> historiesNew = targetDao._queryHistory_Histories(taskId);
            synchronized (this) {
                if (histories == null) {
    ...
    public List<Temporary> getTemporary() {
        if (temporary == null) {
            ...
            List<Temporary> temporaryNew = targetDao._queryHistory_Temporary(taskId);
            synchronized (this) {
                if (temporary == null) {
    ...
    /** Resets a to-many relationship, making the next get call to query for a fresh result. */
    @Generated(hash = 2040322471)
    public synchronized void resetTemporary() {
        temporary = null;
    }
    /**
